...
class DataArguments:
    ...
    max_positive_num: int = field(
        default=-1, metadata={"help": "max positive passages to use in multiple positive setting"}
    )
    positive_type: str = field(
        default="single", 
        metadata=
        {
            "help": "choose the way to select positive passages.", 
            "choices": 
            [
                "single", 
                "random_one",
...
class TevatronTrainingArguments(TrainingArguments):
    ...
    loss_type: str = field(
        default="infonce",
        metadata={
            "help": "Loss function to use. One of 'infonce', 'random_one_lh', 'lsep', 'joint_lh', 'sum_mar'.",
            "choices": ["infonce", "random_one_lh", "lsep", "joint_lh", "sum_mar"]
        }
    )
    normalize_loss_by_positive: bool = field(
        default=False,
        metadata={
            "help": "If true, normalize multi-positive losses by the number of positives per query."
        }
    )
    pair_wise_positive_selector: str = field(
        default="all",
        metadata={
            "help": "Positive selection strategy for pair_wise+lsep loss.",
            "choices": ["min", "max", "all"]
        }
    )
    pair_wise_negative_selector: str = field(
        default="all",
        metadata={
            "help": "Negative selection strategy for pair_wise+lsep loss.",
            "choices": ["max", "all"]
        }
    )
