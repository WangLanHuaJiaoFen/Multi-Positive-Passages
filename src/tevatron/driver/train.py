...
def main():
    ...
    if (
        ...
        # Refuse to overwrite a non-empty output directory unless the user has
        # explicitly allowed it or a checkpoint can be resumed.
        if last_checkpoint is None and os.listdir(training_args.output_dir):
            raise ValueError(
                f"Output directory ({training_args.output_dir}) already exists and is not empty. "
                "Use --overwrite_output_dir to overcome or pass --resume_from_checkpoint to resume."
            )
    ...
    num_labels = 1 # Dense retrieval uses a single scalar similarity score.
    ...
    # The dataset uses trainer state, such as the current epoch, to make
    # deterministic positive and negative sampling decisions.
